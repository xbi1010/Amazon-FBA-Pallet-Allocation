...
def read_excel_sheet_with_columns(source: Any, required_columns: set[str], preferred_sheet: str | None=None) -> tuple[pd.DataFrame, str]:
...
def load_order(order_source: Any) -> pd.DataFrame:
    required = {ORDER_ARN, ORDER_MODEL, ORDER_UPC, ORDER_QTY, ORDER_PALLETS, ORDER_METHOD}
    df, _ = read_excel_sheet_with_columns(order_source, required, preferred_sheet='AFC打托')
    useful = df[ORDER_ARN].notna() | df[ORDER_MODEL].notna() | df[ORDER_UPC].notna() | df[ORDER_QTY].notna()
    ...
    invalid_qty = df[df['_qty_num'].isna() | (df['_qty_num'] <= 0) | ((df['_qty_num'] % 1).abs() > 1e-09)]
...
def load_product_master(product_source: Any) -> tuple[pd.DataFrame, pd.DataFrame]:
    required = {PRODUCT_SKU, PRODUCT_WEIGHT_KG, PRODUCT_LENGTH_CM, PRODUCT_WIDTH_CM, PRODUCT_HEIGHT_CM}
    df, _ = read_excel_sheet_with_columns(product_source, required, preferred_sheet='exportProductDataDetail')
    df = df.copy()
    df['_sku_key'] = df[PRODUCT_SKU].map(normalize_code)
    df = df[df['_sku_key'] != ''].copy()
    numeric_cols = [PRODUCT_WEIGHT_KG, PRODUCT_LENGTH_CM, PRODUCT_WIDTH_CM, PRODUCT_HEIGHT_CM]
    ...
    for sku_key, group in df.groupby('_sku_key', sort=False):
        ...
        if valid.empty:
            chosen = group.iloc[-1]
            warnings.append({'SKU/UPC': sku_key, '类型': '产品资料缺少尺寸或重量', '说明': '该 SKU 存在于 Product.xlsx，但重量/长/宽/高至少有一项无法读取。'})
            chosen_rows.append(chosen)
            continue
        unique_dims = valid[numeric_cols].round(8).drop_duplicates()
        if len(unique_dims) > 1:
            warnings.append({'SKU/UPC': sku_key, '类型': '重复 SKU 且尺寸不一致', '说明': '程序将优先使用最后更新时间最新的一条产品资料。请人工核对 Product.xlsx。'})
    ...
    product['Carton_Volume_in3'] = product['Length_in'] * product['Width_in'] * product['Height_in']
...
def enrich_order_with_product(order: pd.DataFrame, product: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, order_row in order.iterrows():
        ...
        if product_row is None:
            record.update({'_matched': False, '_match_method': '未匹配', '_product_sku_key': '', '_weight_lb': math.nan, '_length_in': math.nan, '_width_in': math.nan, '_height_in': math.nan, '_carton_volume_in3': math.nan})
        else:
            record.update({'_matched': True, '_match_method': match_method, '_product_sku_key': product_row['_sku_key'], '_weight_lb': float(product_row['Weight_lb']), '_length_in': float(product_row['Length_in']), '_width_in': float(product_row['Width_in']), '_height_in': float(product_row['Height_in']), '_carton_volume_in3': float(product_row['Carton_Volume_in3'])})
...
def _validate_product_dimensions(group: pd.DataFrame, arn: str) -> None:
    ...
    if not unmatched.empty:
        ...
        for _, row in unmatched.iterrows():
            items.append(f"Excel第{int(row['_row_no'])}行 / Model={row.get(ORDER_MODEL, '')} / UPC={row.get(ORDER_UPC, '')}")
        raise AllocationError('产品资料匹配失败：' + '；'.join(items))
    invalid = group[group['_carton_volume_in3'].isna() | (group['_carton_volume_in3'] <= 0) | group['_weight_lb'].isna()]
    if not invalid.empty:
        raise AllocationError(f'ARN {arn} 的产品资料存在无效尺寸或重量。')

def _base_detail_record(row: pd.Series, arn: str, pallet_no: int, method_label: str, allocated_boxes: int) -> dict[str, Any]:
    carton_volume = float(row['_carton_volume_in3'])
    weight_lb = float(row['_weight_lb'])
    return {'Shipment ID (ARN)': arn, 'Pallet No.': pallet_no, '打托方式': method_label, 'Order/PO Number': row.get(ORDER_PO, ''), 'Fulfillment Center': row.get(ORDER_FC, ''), 'Model Number': row.get(ORDER_MODEL, ''), 'UPC/SKU': row.get(ORDER_UPC, ''), '产品匹配方式': row.get('_match_method', ''), 'Allocated Boxes': int(allocated_boxes), 'Weight per Box (lb)': weight_lb, 'Length (in)': float(row['_length_in']), 'Width (in)': float(row['_width_in']), 'Height (in)': float(row['_height_in']), 'Volume per Box (in³)': carton_volume, 'Allocated Weight (lb)': allocated_boxes * weight_lb, 'Allocated Volume (in³)': allocated_boxes * carton_volume}

def allocate_single_sku_pallets(group: pd.DataFrame, arn: str) -> tuple[list[dict[str, Any]], int]:
    work = group.copy()
    missing_pallets = work[work['_pallets_num'].isna() | (work['_pallets_num'] <= 0) | ((work['_pallets_num'] % 1).abs() > 1e-09)]
    if not missing_pallets.empty:
        rows = ', '.join(map(str, missing_pallets['_row_no'].astype(int).tolist()))
        raise AllocationError(f'single sku pallet 模式要求每个 SKU 行都填写正整数 Pallets。问题行：{rows}')
    ...
    for row in aggregated:
        ...
        if qty < pallet_count:
            raise AllocationError(f"SKU {row.get(ORDER_MODEL, '')} 只有 {qty} 箱，但要求 {pallet_count} 个 single-SKU 托盘，会产生空托。")
        ...
        if max_volume > PALLET_CAPACITY_IN3 + 1e-06:
            ...
            raise AllocationError(f"SKU {row.get(ORDER_MODEL, '')} 按 {pallet_count} 托平均分配后单托体积会超限。当前最大单托 {max_boxes} 箱，约 {max_volume:,.0f} in³；容量上限 {PALLET_CAPACITY_IN3:,.0f} in³。按体积至少需要 {min_pallets_by_volume} 托。")
        for count in counts:
            details.append(_base_detail_record(row=row, arn=arn, pallet_no=pallet_no, method_label='single sku pallet', allocated_boxes=count))
            pallet_no += 1
    return (details, pallet_no - 1)

def _solve_mixed_distribution(sku_rows: list[pd.Series], pallet_count: int, time_limit_seconds: float=4.0) -> list[list[int]]:
    sku_count = len(sku_rows)
    quantities = [int(row['_qty_num']) for row in sku_rows]
    volume_units = [max(1, int(round(float(row['_carton_volume_in3']) * VOLUME_SCALE))) for row in sku_rows]
    ...
    if sum(quantities) < 2 * pallet_count:
        raise AllocationError(f'混合打托要求每托至少 2 个不同 SKU，但总箱数只有 {sum(quantities)}，不足以覆盖 {pallet_count} 托。')
    presence_capacity = sum((min(qty, pallet_count) for qty in quantities))
    if presence_capacity < 2 * pallet_count:
        raise AllocationError('即使把每个 SKU 尽可能分散，也无法保证每个托盘至少包含 2 个不同 SKU。')
    ...
    if total_volume_units > pallet_count * capacity_units:
        ...
        raise AllocationError(f'总体积 {total_volume:,.0f} in³ 超过 {pallet_count} 托的总容量 {pallet_count * PALLET_CAPACITY_IN3:,.0f} in³，固定托数下无法完成。')
    for i, v in enumerate(volume_units):
        if v > capacity_units:
            raise AllocationError(f"SKU {sku_rows[i].get(ORDER_MODEL, '')} 单箱体积已经超过一个托盘容量。")
    ...
    if strict_status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        for i in range(sku_count):
            row_counts: list[int] = []
            for p in range(pallet_count):
                extra = int(strict_solver.Value(extra_vars[i, p])) if (i, p) in extra_vars else 0
    ...
    for p in range(pallet_count):
        ...
        model.Add(volume_var == sum((volume_units[i] * x[i, p] for i in range(sku_count))))
    ...
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        raise AllocationError('在固定托数、每托至少 2 个 SKU、且单托体积不超限的条件下，没有找到可行分配。')
    phase1_solution = [[int(solver.Value(x[i, p])) for p in range(pallet_count)] for i in range(sku_count)]
    ...
    return [[int(solver2.Value(x[i, p])) for p in range(pallet_count)] for i in range(sku_count)]
...
def allocate_mixed_pallets(group: pd.DataFrame, arn: str) -> tuple[list[dict[str, Any]], int]:
    ...
    invalid = pallet_values[(pallet_values <= 0) | ((pallet_values % 1).abs() > 1e-09)]
    if not invalid.empty:
        raise AllocationError('混合打托的 Pallets 必须是正整数。')
    unique_counts = sorted({int(v) for v in pallet_values})
    if len(unique_counts) != 1:
        raise AllocationError(f'同一个混合打托 ARN 出现多个不同的 Pallets 值：{unique_counts}。混合打托应填写一个固定的 ARN 总托数。')
    ...
    for sku_index, row in enumerate(sku_rows):
        for pallet_index in range(pallet_count):
            ...
            if qty <= 0:
                continue
            details.append(_base_detail_record(row=row, arn=arn, pallet_no=pallet_index + 1, method_label='混合打托', allocated_boxes=qty))
    detail_df = pd.DataFrame(details)
    if detail_df.empty:
        raise AllocationError('混合打托计算结果为空。')
    check = detail_df.groupby('Pallet No.').agg(SKU_Count=('UPC/SKU', 'nunique'), Volume=('Allocated Volume (in³)', 'sum'))
    ...
    if (check['Volume'] > PALLET_CAPACITY_IN3 + 1e-06).any():
...
def allocate_orders(order_source: Any, product_source: Any) -> AllocationResult:
    ...
    for arn, group in order.groupby('_arn_key', sort=False):
        ...
        total_volume = (group['_qty_num'] * group['_carton_volume_in3'].fillna(0)).sum()
        summary = {'Shipment ID (ARN)': arn, 'Order/PO Number': po, 'Fulfillment Center': fc, '打托方式': methods[0] if len(methods) == 1 else ' / '.join(methods), 'SKU Count': int(group['_product_sku_key'].replace('', pd.NA).nunique()), 'Total Boxes': total_boxes, 'Required / Generated Pallets': None, 'Total Volume (in³)': float(total_volume), 'Total Capacity (in³)': None, 'Overall Utilization': None, 'Status': 'FAILED', 'Message': ''}
        try:
            if len(methods) != 1:
                raise AllocationError(f"同一个 ARN 必须有且只有一种打托方式。当前读取到：{methods or ['空白']}")
            ...
            source_qty = group.groupby('_product_sku_key', sort=False)['_qty_num'].sum().to_dict()
            detail_df = pd.DataFrame(details)
            ...
            for source_key, qty in source_qty.items():
                if allocated_qty.get(source_key, 0) != int(qty):
                    raise AllocationError(f'内部数量校验失败：SKU {source_key} 订单 {qty} 箱，分配 {allocated_qty.get(source_key, 0)} 箱。')
            all_details.extend(details)
            capacity = pallet_count * PALLET_CAPACITY_IN3
            summary.update({'Required / Generated Pallets': pallet_count, 'Total Capacity (in³)': capacity, 'Overall Utilization': float(total_volume / capacity) if capacity else None, 'Status': 'OK', 'Message': '分配完成'})
        except Exception as exc:
            message = str(exc)
            summary['Message'] = message
            issues.append({'Shipment ID (ARN)': arn, 'Order/PO Number': po, 'Fulfillment Center': fc, '打托方式': summary['打托方式'], '错误说明': message})
        arn_summaries.append(summary)
    detail_columns = ['Shipment ID (ARN)', 'Pallet No.', '打托方式', 'Order/PO Number', 'Fulfillment Center', 'Model Number', 'UPC/SKU', '产品匹配方式', 'Allocated Boxes', 'Weight per Box (lb)', 'Length (in)', 'Width (in)', 'Height (in)', 'Volume per Box (in³)', 'Allocated Weight (lb)', 'Allocated Volume (in³)']
    detail_df = pd.DataFrame(all_details, columns=detail_columns)
    if detail_df.empty:
        pallet_summary = pd.DataFrame(columns=['Shipment ID (ARN)', 'Pallet No.', '打托方式', 'SKU Count', 'Total Boxes', 'Total Weight (lb)', 'Total Volume (in³)', 'Pallet Capacity (in³)', 'Volume Utilization', 'Status'])
    else:
        pallet_summary = detail_df.groupby(['Shipment ID (ARN)', 'Pallet No.', '打托方式'], sort=False, as_index=False).agg(**{'SKU Count': ('UPC/SKU', 'nunique'), 'Total Boxes': ('Allocated Boxes', 'sum'), 'Total Weight (lb)': ('Allocated Weight (lb)', 'sum'), 'Total Volume (in³)': ('Allocated Volume (in³)', 'sum')})
        pallet_summary['Pallet Capacity (in³)'] = PALLET_CAPACITY_IN3
        pallet_summary['Volume Utilization'] = pallet_summary['Total Volume (in³)'] / PALLET_CAPACITY_IN3
        pallet_summary['Status'] = 'OK'
        pallet_summary.loc[pallet_summary['Total Volume (in³)'] > PALLET_CAPACITY_IN3 + 1e-06, 'Status'] = 'OVER VOLUME'
        mixed_bad = pallet_summary['打托方式'].eq('混合打托') & (pallet_summary['SKU Count'] < 2)
        pallet_summary.loc[mixed_bad, 'Status'] = 'LESS THAN 2 SKUs'
    arn_summary_df = pd.DataFrame(arn_summaries)
    issues_df = pd.DataFrame(issues, columns=['Shipment ID (ARN)', 'Order/PO Number', 'Fulfillment Center', '打托方式', '错误说明'])
    return AllocationResult(detail=detail_df, pallet_summary=pallet_summary, arn_summary=arn_summary_df, issues=issues_df, product_warnings=product_warnings)
...
def build_output_excel(result: AllocationResult) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        detail.to_excel(writer, sheet_name='Pallet Allocation', index=False)
        if not result.issues.empty:
            errors = result.issues.rename(columns={'Shipment ID (ARN)': 'ARN', '错误说明': 'Error'})
            errors.to_excel(writer, sheet_name='Errors', index=False)
        workbook = writer.book
        for ws in workbook.worksheets:
            _style_worksheet(ws)
        ws = workbook['Pallet Allocation']
